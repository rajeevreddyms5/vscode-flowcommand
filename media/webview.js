...
(function () {
  ...
  let soundToggle,
    ...
    autoFocusPanelToggle,
    mobileNotificationToggle,
    interactiveApprovalToggle;
  let instructionInjectionSelect,
    instructionTextArea,
    instructionTextSaveBtn,
    instructionInjectBtn,
    instructionRemoveBtn,
    instructionResetBtn,
    instructionReinjectBtn,
    instructionStatus;
  ...
  function init() {
    try {
      ...
      console.log(
        "[FlowCommand Webview] Event listeners bound, pendingMessage element:",
        !!pendingMessage,
      );
      renderQueue();
      updateModeUI();
  ...
  function createSettingsModal() {
    ...
    reportBtn.addEventListener("click", function () {
      vscode.postMessage({
        type: "openExternal",
        url: "https://github.com/rajeevreddyms5/vscode-flowcommand/issues/new",
      });
    });
    ...
    notificationsGroup.innerHTML =
      ...
      '<div class="settings-section"><div class="settings-section-header">' +
      '<div class="settings-section-title"><span class="codicon codicon-bell"></span> IDE Notification</div>' +
      '<div class="toggle-switch active" id="desktop-notification-toggle" role="switch" aria-checked="true" aria-label="Enable IDE notification popup" tabindex="0"></div>' +
      "</div></div>" +
      // Auto-Focus Panel
      '<div class="settings-section"><div class="settings-section-header">' +
    ...
    desktopNotificationToggle = document.getElementById(
      "desktop-notification-toggle",
    );
    autoFocusPanelToggle = document.getElementById("auto-focus-panel-toggle");
    mobileNotificationToggle = document.getElementById(
      "mobile-notification-toggle",
    );
    interactiveApprovalToggle = document.getElementById(
      "interactive-approval-toggle",
    );
    instructionInjectionSelect = document.getElementById(
      "instruction-injection-select",
    );
    instructionTextArea = document.getElementById("instruction-text-area");
    instructionTextSaveBtn = document.getElementById(
      "instruction-text-save-btn",
    );
    instructionInjectBtn = document.getElementById("instruction-inject-btn");
    instructionReinjectBtn = document.getElementById(
      "instruction-reinject-btn",
    );
  ...
  function createPromptsModal() {
    ...
    titleSpan.innerHTML =
      '<span class="codicon codicon-symbol-keyword"></span> Reusable Prompts';
  ...
  function bindEventListeners() {
    ...
    console.log(
      "[FlowCommand] Setting up drag-drop, dropTarget found:",
      !!dropTarget,
    );
    if (dropTarget) {
      ...
      function hasDragFiles(dt) {
        ...
        console.log(
          "[FlowCommand] hasDragFiles: types =",
          Array.from(dt.types).join(", "),
        );
    ...
    if (queuePauseBtn)
      queuePauseBtn.addEventListener("click", handleQueuePauseClick);
    if (queueClearBtn)
      queueClearBtn.addEventListener("click", handleQueueClearClick);
    ...
    var notificationsGroupToggle = document.getElementById(
    ...
    var instructionGroupToggle = document.getElementById(
      "instruction-group-toggle",
    );
    ...
    if (desktopNotificationToggle) {
      desktopNotificationToggle.addEventListener(
        "click",
        toggleDesktopNotificationSetting,
      );
    ...
    if (autoFocusPanelToggle) {
      autoFocusPanelToggle.addEventListener(
        "click",
        toggleAutoFocusPanelSetting,
      );
    ...
    if (mobileNotificationToggle) {
      mobileNotificationToggle.addEventListener(
        "click",
        toggleMobileNotificationSetting,
      );
    ...
    if (promptsModalClose)
      promptsModalClose.addEventListener("click", closePromptsModal);
    ...
    if (promptsModalAddBtn)
      promptsModalAddBtn.addEventListener("click", showPromptsModalAddForm);
    var pmCancelBtn = document.getElementById("pm-cancel-btn");
    var pmSaveBtn = document.getElementById("pm-save-btn");
    if (pmCancelBtn)
      pmCancelBtn.addEventListener("click", hidePromptsModalAddForm);
    ...
    if (instructionInjectBtn) {
      instructionInjectBtn.addEventListener("click", function () {
        if (instructionInjectionSelect) {
          vscode.postMessage({
            type: "updateInstructionInjection",
            method: instructionInjectionSelect.value,
          });
        }
      });
    }
    ...
    window.dispatchVSCodeMessage = function (message) {
      console.log(
        "[FlowCommand Webview] dispatchVSCodeMessage called:",
        message.type,
      );
      handleExtensionMessage({ data: message });
    };
    if (
      window.__flowcommandInitialState &&
      typeof window.__applyFlowCommandInitialState === "function"
    ) {
  ...
  function updateInputHighlighter() {
    ...
    var knownSlashNames = reusablePrompts.map(function (p) {
      return p.name;
    });
  ...
  function handleQueueHeaderClick(e) {
    ...
    if (
      e.target.closest(".queue-pause-btn") ||
      e.target.closest(".queue-clear-btn")
    )
      return;
  ...
  function showPendingToolCall(id, prompt, isApproval, choices, context) {
    console.log(
      "[FlowCommand Webview] showPendingToolCall called with id:",
      id,
    );
    ...
    if (pendingMessage) {
      ...
      console.log(
        "[FlowCommand Webview] pendingMessage.innerHTML set, length:",
        pendingMessage.innerHTML.length,
      );
    } else {
  ...
  function addToolCallToCurrentSession(entry) {
    ...
    var idx = currentSessionCalls.findIndex(function (tc) {
  ...
  function startProcessingTimeout() {
    ...
    processingTimeoutId = setTimeout(function () {
      if (isProcessingResponse) {
        console.log(
          "[FlowCommand] Processing timeout - auto-clearing stuck processing state",
        );
        clearProcessingState();
      }
  ...
  function renderCurrentSession() {
    ...
    var completedCalls = currentSessionCalls.filter(function (tc) {
  ...
  function processTableBuffer(lines, maxRows) {
    ...
    // Parse header
    var headerCells = lines[0].split("|").filter(function (c) {
      return c.trim() !== "";
    });
  ...
  function updateWelcomeSectionVisibility() {
    ...
    var hasCompletedCalls = currentSessionCalls.some(function (tc) {
      return tc.status === "completed";
    });
    var hasPendingMessage =
  ...
  function startEditPrompt(id) {
    ...
    var item = promptQueue.find(function (p) {
      return p.id === id;
    });
    if (!item) return;
    ...
    var queueItem = queueList.querySelector(
  ...
  function toggleDesktopNotificationSetting() {
    ...
    updateDesktopNotificationToggleUI();
    vscode.postMessage({
      type: "updateDesktopNotificationSetting",
  ...
  function updateDesktopNotificationToggleUI() {
    ...
    desktopNotificationToggle.classList.toggle(
      "active",
      desktopNotificationEnabled,
    );
    desktopNotificationToggle.setAttribute(
      "aria-checked",
      desktopNotificationEnabled ? "true" : "false",
    );
  }
  ...
  function toggleAutoFocusPanelSetting() {
    ...
    updateAutoFocusPanelToggleUI();
    vscode.postMessage({
      type: "updateAutoFocusPanelSetting",
      enabled: autoFocusPanelEnabled,
    });
  }
  ...
  function updateAutoFocusPanelToggleUI() {
    ...
    autoFocusPanelToggle.setAttribute(
      "aria-checked",
      autoFocusPanelEnabled ? "true" : "false",
    );
  }
  ...
  function toggleMobileNotificationSetting() {
    ...
    vscode.postMessage({
      type: "updateMobileNotificationSetting",
      enabled: mobileNotificationEnabled,
    });
  }

  function updateMobileNotificationToggleUI() {
    if (!mobileNotificationToggle) return;
    mobileNotificationToggle.classList.toggle(
      "active",
      mobileNotificationEnabled,
    );
    mobileNotificationToggle.setAttribute(
      "aria-checked",
      mobileNotificationEnabled ? "true" : "false",
    );
  }
  ...
  function updateInstructionUI() {
    ...
    if (instructionReinjectBtn) {
      var needsReinject =
        instructionInjection !== "off" &&
        (instructionState === "modified" ||
          instructionState === "missing" ||
          instructionState === "corrupted" ||
          instructionState === "no-file");
      instructionReinjectBtn.style.display = needsReinject
        ? "inline-flex"
        : "none";
    }
    if (instructionStatus) {
      if (instructionInjection === "off") {
        instructionStatus.innerHTML =
          '<span style="color:var(--vscode-descriptionForeground);">Status: Not injected</span>';
      } else if (instructionState === "correct") {
        if (instructionInjection === "copilotInstructionsMd") {
          instructionStatus.innerHTML =
            '<span style="color:var(--vscode-charts-green, #89d185);"><span class="codicon codicon-check"></span> Injected into .github/copilot-instructions.md (workspace)</span>';
        } else if (instructionInjection === "codeGenerationSetting") {
          instructionStatus.innerHTML =
            '<span style="color:var(--vscode-charts-green, #89d185);"><span class="codicon codicon-check"></span> Injected into Code Generation settings (workspace)</span>';
        }
      } else if (instructionState === "modified") {
        instructionStatus.innerHTML =
          '<span style="color:var(--vscode-charts-yellow, #d7ba7d);"><span class="codicon codicon-warning"></span> Instructions were modified. Re-inject to restore FlowCommand defaults.</span>';
      } else if (
        instructionState === "missing" ||
        instructionState === "no-file"
      ) {
        instructionStatus.innerHTML =
          '<span style="color:var(--vscode-charts-yellow, #d7ba7d);"><span class="codicon codicon-warning"></span> Instructions missing. Re-inject to restore FlowCommand defaults.</span>';
      } else if (instructionState === "corrupted") {
        instructionStatus.innerHTML =
          '<span style="color:var(--vscode-errorForeground);"><span class="codicon codicon-error"></span> Instruction markers corrupted. Re-inject to fix.</span>';
      } else {
        instructionStatus.innerHTML =
          '<span style="color:var(--vscode-descriptionForeground);">Status: Unknown</span>';
      }
    }
  }
  ...
  function updateMcpUI() {
    if (mcpStatusText) {
      if (mcpRunning) {
        mcpStatusText.innerHTML =
          '<span style="color:var(--vscode-charts-green, #89d185);"><span class="codicon codicon-check"></span> Running</span>';
      } else {
        mcpStatusText.innerHTML =
          '<span style="color:var(--vscode-descriptionForeground);">Stopped</span>';
      }
    }
    if (mcpUrlText) {
  ...
  function showPlanReviewModal(reviewId, title, plan) {
    ...
    overlay.innerHTML =
      ...
      '<span class="plan-review-title">' +
      escapeHtml(title || "Plan Review") +
      "</span>" +
      '<span class="plan-review-badge">Review</span>' +
      '<button class="plan-review-close-btn" id="pr-close-' +
      reviewId +
      '" title="Cancel review"><span class="codicon codicon-close"></span></button>' +
      "</div>" +
      // Split content area: plan left, comments right
      '<div class="plan-review-content">' +
      '<div class="plan-review-body" id="pr-body-' +
      reviewId +
      '"></div>' +
      '<div class="plan-review-sidebar" id="pr-sidebar-' +
      reviewId +
      '">' +
      '<div class="plan-review-comments-header">' +
      'Comments <span class="plan-review-comments-count" id="pr-count-' +
      reviewId +
      '">0</span>' +
      '<button class="plan-review-clear-all hidden" id="pr-clear-all-' +
      reviewId +
      '" title="Clear all comments">Clear All</button>' +
      "</div>" +
      '<div class="plan-review-comments-list" id="pr-clist-' +
      reviewId +
      '">' +
      ...
      '<textarea class="form-input form-textarea" id="pr-comment-input-' +
      reviewId +
      '" placeholder="Your feedback or revision instructions..." rows="2"></textarea>' +
      '<input class="form-input" id="pr-comment-part-' +
      reviewId +
      '" placeholder="Which part? (e.g., Step 3)" />' +
      '<button class="form-btn form-btn-save" id="pr-add-comment-' +
      reviewId +
      '">Add Comment</button>' +
      "</div>" +
      "</div>" +
      ...
      '<button class="form-btn form-btn-cancel" id="pr-cancel-' +
      reviewId +
      '">Cancel</button>' +
      '<div class="plan-review-footer-right">' +
      '<button class="form-btn form-btn-cancel" id="pr-reject-' +
      reviewId +
      '" disabled>Request Changes</button>' +
      '<button class="form-btn form-btn-save" id="pr-approve-' +
      reviewId +
      '">Approve</button>' +
      "</div>" +
      "</div>" +
    ...
    if (bodyEl) {
      ...
      elements.forEach(function (el) {
        ...
        if (
          el.parentElement &&
          el.parentElement.classList.contains("pr-line-wrapper")
        )
          return;
        ...
        commentBtn.addEventListener("click", function (e) {
          e.stopPropagation();
          var partInput = document.getElementById(
            "pr-comment-part-" + reviewId,
          );
          var commentInput = document.getElementById(
            "pr-comment-input-" + reviewId,
          );
    ...
    if (addCommentBtn) {
      addCommentBtn.addEventListener("click", function () {
        ...
        comments.push({
          revisedPart: part || "(general)",
          revisorInstructions: instruction,
        });
        commentInput.value = "";
        partInput.value = "";
        renderPlanReviewComments(reviewId, comments);
  ...
  function updatePlanReviewButtons(reviewId, comments) {
    ...
    if (approveBtn)
      approveBtn.textContent =
        comments.length > 0 ? "Approve with Comments" : "Approve";
    if (clearAllBtn) {
  ...
  function updatePlanReviewLineHighlights(reviewId, comments) {
    ...
    bodyEl
      .querySelectorAll(".pr-line-wrapper.has-comment")
      .forEach(function (el) {
    ...
    comments.forEach(function (comment) {
      bodyEl.querySelectorAll(".pr-line-wrapper").forEach(function (wrapper) {
        ...
        if (
          wrapperText &&
          comment.revisedPart.includes(wrapperText.substring(0, 50))
        ) {
  ...
  function renderPlanReviewComments(reviewId, comments) {
    ...
    listEl.innerHTML = comments
      .map(function (c, i) {
        return (
          '<div class="plan-review-comment-item">' +
          '<div class="pr-comment-citation">' +
          escapeHtml(c.revisedPart) +
          "</div>" +
          '<div class="pr-comment-text">' +
          escapeHtml(c.revisorInstructions) +
          "</div>" +
          '<div class="pr-comment-actions">' +
          '<button class="pr-remove-comment" data-index="' +
          i +
          '">Remove</button>' +
          "</div>" +
          "</div>"
  ...
  function closePlanReviewModal(reviewId) {
    console.log(
      "[FlowCommand] closePlanReviewModal called with:",
      reviewId,
      "activePlanReview:",
  ...
  function showMultiQuestionModal(requestId, questions) {
    ...
    if (
      ...
      console.error(
        "[FlowCommand] showMultiQuestionModal: invalid input",
        requestId,
        questions,
      );
    ...
    var questionsHtml = safeQuestions
      .map(function (q, qIndex) {
        var optionsHtml = "";
        ...
        if (q.options && q.options.length > 0) {
          ...
          optionsHtml = q.options
            .map(function (opt, oIndex) {
              var recommendedBadge = opt.recommended
                ? '<span class="mq-recommended">recommended</span>'
                : "";
              var description = opt.description
                ? '<span class="mq-option-desc">' +
                  escapeHtml(opt.description) +
                  "</span>"
                : "";
              return (
                '<label class="mq-option' +
                (opt.recommended ? " mq-option-recommended" : "") +
                '">' +
                '<input type="' +
                inputType +
                '" name="mq-q' +
        ...
        if (!q.options || q.options.length === 0 || q.allowFreeformInput) {
          var placeholder =
            q.options && q.options.length > 0
              ...
              : "Type your answer...";
          freeformHtml =
            '<textarea class="mq-freeform" id="mq-freeform-' +
            qIndex +
            '" data-qindex="' +
            qIndex +
            '" placeholder="' +
        ...
        return (
          '<div class="mq-question" data-qindex="' +
          qIndex +
          '">' +
          '<div class="mq-question-header">' +
          '<span class="mq-question-number">' +
          (qIndex + 1) +
          "</span>" +
          '<span class="mq-question-label">' +
          escapeHtml(q.header) +
          "</span>" +
          "</div>" +
    ...
    if (pendingMessage) {
      pendingMessage.classList.remove("hidden");
      pendingMessage.innerHTML =
        '<div class="mq-inline-container" id="mq-container-' +
        requestId +
        '">' +
        ...
        "</div>" +
        '<div class="mq-inline-content">' +
        questionsHtml +
        "</div>" +
        '<div class="mq-inline-footer">' +
        '<button class="form-btn form-btn-cancel" id="mq-cancel-' +
        requestId +
        '">Cancel</button>' +
        '<button class="form-btn form-btn-save" id="mq-submit-' +
        requestId +
        '">Submit</button>' +
        "</div>" +
        "</div>";
      ...
      container
        ...
        .forEach(function (input) {
          input.addEventListener("change", function () {
            ...
            if (isNaN(qIndex) || qIndex < 0 || qIndex >= safeQuestions.length)
              return;
            ...
            if (safeQuestions[qIndex].multiSelect) {
              ...
              if (input.checked) {
                ...
                if (isOther) {
                  ...
                  answers[qIndex].selected = answers[qIndex].selected.filter(
                    function (v) {
                      return !v.startsWith("Other: ");
                    },
                  );
                } else {
                  answers[qIndex].selected = answers[qIndex].selected.filter(
  ...
  function closeMultiQuestionModal(requestId) {
    if (!activeMultiQuestion || activeMultiQuestion.requestId !== requestId)
      return;
  ...
  function renderPromptsModalList() {
    ...
    if (reusablePrompts.length === 0) {
      promptsModalList.innerHTML =
        '<div class="prompts-modal-empty"><span class="codicon codicon-symbol-keyword"></span><p>No prompts yet</p><p class="prompts-modal-empty-hint">Add a prompt with the <span class="codicon codicon-add"></span> button above.<br>Use <code>/name</code> in the input to expand it.</p></div>';
      return;
    }

    promptsModalList.innerHTML = reusablePrompts
      .map(function (p, index) {
        ...
        return (
          ...
          (p.isTemplate ? " is-template" : "") +
          '" data-id="' +
          escapeHtml(p.id) +
          '">' +
          '<div class="prompt-card-header">' +
          '<span class="prompt-card-name">/' +
          escapeHtml(p.name) +
          "</span>" +
          '<span class="prompt-card-preview">' +
          escapeHtml(promptPreview) +
          "</span>" +
          '<div class="prompt-card-actions-inline">' +
          '<button class="prompt-card-btn-icon ' +
          templateBtnClass +
          '" data-id="' +
          escapeHtml(p.id) +
          '" title="' +
          templateBtnTitle +
          '"><span class="codicon codicon-pinned"></span></button>' +
          '<button class="prompt-card-btn-icon pm-edit-btn" data-id="' +
          escapeHtml(p.id) +
          '" title="Edit"><span class="codicon codicon-edit"></span></button>' +
          '<button class="prompt-card-btn-icon pm-delete-btn" data-id="' +
          escapeHtml(p.id) +
    ...
    promptsModalList
      .querySelectorAll(".pm-template-btn")
      .forEach(function (btn) {
        btn.addEventListener("click", function (e) {
          e.stopPropagation();
          var id = btn.getAttribute("data-id");
  ...
  function editPromptInModal(id) {
    var prompt = reusablePrompts.find(function (p) {
      return p.id === id;
    });
    if (!prompt) return;
  ...
  function updateTemplateIndicator() {
    ...
    var activeTemplate = reusablePrompts.find(function (p) {
      return p.isTemplate === true;
    });
    ...
    if (!indicator) {
      ...
      indicator.innerHTML =
        '<span class="codicon codicon-pinned"></span> <span class="template-name"></span> <button class="template-clear-btn" title="Disable template"><span class="codicon codicon-close"></span></button>';
      inputWrapper.insertBefore(indicator, inputWrapper.firstChild);

      // Bind clear button
      indicator
        .querySelector(".template-clear-btn")
  ...
  function expandSlashCommands(text) {
    ...
    var promptNames = reusablePrompts.map(function (p) {
      return p.name;
    });
  ...
  function selectSlashItem(index) {
    ...
    chatInput.value =
      value.substring(0, slashStartPos) +
      slashText +
  ...
  function playNotificationSound() {
    console.log(
      "[FlowCommand] playNotificationSound called, audioUnlocked:",
      audioUnlocked,
    );
      ...
      if (audio && audio.src && !audio.error) {
        ...
        if (playPromise !== undefined) {
          playPromise
            ...
            .catch(function (e) {
              console.log(
                "[FlowCommand] Could not play audio file:",
                e.message,
              );
  ...
  function showAutocomplete(results) {
    if (!autocompleteDropdown || !autocompleteList || !autocompleteEmpty)
      return;
  ...
  function scrollToSelectedItem() {
    var selectedItem = autocompleteList
