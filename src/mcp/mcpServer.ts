...
import { askUser, Question } from "../tools";
...
async function tryReadImageAsMcpContent(
  ...
  try {
    ...
    if (stat.size > MAX_IMAGE_BYTES) {
      console.warn(
        `[FlowCommand MCP] Skipping image >4MB: ${filePath} (${stat.size} bytes)`,
      );
      return null;
    }
    ...
    try {
        ...
        "ask_user",
        {
          ...
          inputSchema: z.object({
            ...
            question: z
              ...
              .optional()
              .describe(
                "The question or prompt to display to the user (for modes A and B only)",
              ),
            choices: z
              .array(
                z.object({
                  label: z
                    .string()
                    .describe("Display text for the choice button"),
                  value: z
                    .string()
                    .describe("Value sent back when this choice is selected"),
                }),
              )
              .optional()
              .describe(
                "Optional list of choices to display as clickable buttons (mode A only)",
              ),
            questions: z
              .array(
                z.object({
                  header: z
                    .string()
                    .max(12, "Header must be 12 chars or less")
                    .describe("Short label displayed as question header"),
                    ...
                    .array(
                      z.object({
                        ...
                        recommended: z
                          .boolean()
                          .optional()
                          .describe("Mark as recommended"),
                      }),
                    )
                    .optional()
                    .describe("Options for radio/checkbox selection"),
                  multiSelect: z
                    .boolean()
                    .optional()
                    .describe(
          ...
          "plan_review",
          {
            ...
            inputSchema: z.object({
              plan: z
                ...
                .describe(
                  "The detailed plan in Markdown format to present to the user for review. Use headers, bullet points, and code blocks for clarity.",
                ),
              title: z
                .string()
                .optional()
                .describe(
                  "Optional title for the review panel. Defaults to 'Plan Review'.",
                ),
            }),
          },
          async (
            args: { plan: string; title?: string },
            extra: { signal?: AbortSignal },
          ) => {
            const tokenSource = new vscode.CancellationTokenSource();
            if (extra.signal) {
      ...
      await Promise.race([
        ...
        new Promise<void>((_, reject) => {
          setTimeout(
            () => reject(new Error("Server listen timeout after 10 seconds")),
            10000,
          );
        }),
      ]);
      ...
      vscode.window.showErrorMessage(
        `Failed to start FlowCommand MCP server: ${error}`,
      );
    }
  }
    ...
    try {
      ...
      try {
        ...
        if ((e as NodeJS.ErrnoException).code !== "ENOENT") {
          console.warn(
            `[FlowCommand MCP] Failed to parse ${configPath}, starting fresh`,
          );
        }
      }
      ...
      console.error(
        `[FlowCommand MCP] Failed to register with ${configPath}:`,
        error,
      );
    }
  }
