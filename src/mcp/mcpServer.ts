    ...
    async start(reusePort: boolean = false) {
        try {
            ...
            (this.mcpServer as any).registerTool(
                "ask_user",
                {
                    description: "SUBAGENT RESTRICTION: Subagents (via runSubagent) MUST NOT call this tool. MODES — pick exactly one: (A) Single question + choices → pass 'question' + 'choices'. Shows choice BUTTONS above the always-on text input. No modal, no Submit/Cancel. User clicks button OR types custom. Example: { question: 'Which DB?', choices: [{label:'1. PostgreSQL',value:'postgresql'},{label:'2. MySQL',value:'mysql'},{label:'3. SQLite',value:'sqlite'}] }. (B) Single open-ended question → pass 'question' only. Uses the always-on text input. No modal, no Submit/Cancel, no form. Example: { question: 'What would you like to do next?' }. (C) Multi-question form → pass 'questions' array (2-4 items). Shows MODAL with Submit/Cancel. Use ONLY when gathering 2+ pieces of info. Each item needs 'header' (≤12 chars) + 'question'. Add 'options' for radio/checkbox; omit for free text. Set 'multiSelect: true' for checkboxes. Example: { questions: [{header:'Language',question:'What language?',options:[{label:'Python'},{label:'Go'}]},{header:'Framework',question:'What framework?'}] }. RULES: NEVER use 'questions' for single question — use mode A or B. ALWAYS use 'choices' when question has predefined options. ALWAYS populate 'context' with your FULL response (explanations, code, analysis). The user reads this from FlowCommand's remote UI — without it they see nothing. NOTE: For presenting detailed multi-step plans or proposals that need user sign-off, use plan_review instead.",
