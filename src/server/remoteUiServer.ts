            ...
            function applyInitialState(state) {
                ...
                if (window.dispatchVSCodeMessage) {
                    ...
                    if (state.pendingInputCount !== undefined) {
                        ...
                        // Use setTimeout to ensure DOM is ready (especially on page refresh/reconnect)
                        const updateBadge = () => {
                            if (window.updateRemotePendingBadge) {
                                const btn = document.getElementById('remote-pending-badge-btn');
                                const badge = document.getElementById('remote-badge-count');
                                if (btn && badge) {
                                    // Elements exist - update badge
                                    window.updateRemotePendingBadge(state.pendingInputCount);
                                    console.log('[FlowCommand] Badge synced on reconnect:', state.pendingInputCount);
                                } else {
                                    // Elements not ready - retry after a short delay
                                    console.log('[FlowCommand] Badge elements not ready, retrying...');
                                    setTimeout(updateBadge, 100);
                                }
                            }
                        };
                        // Start badge update (with retry logic)
