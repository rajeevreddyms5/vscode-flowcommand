  ...
  implements vscode.WebviewViewProvider, vscode.Disposable
{
  ...
  public saveCurrentSessionToHistory(): void {
    ...
    if (completedCalls.length > 0) {
      ...
      if (newCalls.length > 0) {
        ...
        this._persistedHistory = [...newCalls, ...this._persistedHistory].slice(
          0,
  ...
  public notifyPlanReviewEnded(): void {
    this._pendingPlanReviewCount = Math.max(
      0,
      this._pendingPlanReviewCount - 1,
    );
    this._updateBadge();
  }
  ...
  private _updateBadge(): void {
    ...
    console.log(
      `[FlowCommand] _updateBadge: askUser=${askUserCount}, queued=${queuedCount}, planReview=${this._pendingPlanReviewCount}, total=${total}, toolCallId=${this._currentToolCallId}, pendingReqSize=${this._pendingRequests.size}`,
    );
    ...
    if (
      ...
      console.warn(
        `[FlowCommand] _updateBadge: clearing stale _currentToolCallId ${this._currentToolCallId} (no pending request)`,
      );
    ...
    if (this._view) {
      try {
        if (total > 0) {
          ...
          // Force clear by setting to 0 explicitly first, then undefined
          // This ensures VS Code updates the badge display even with caching/timing issues
          try {
            this._view.badge = { value: 0, tooltip: "" };
          } catch {
            // Ignore intermediate set error
          }
          // Small delay to ensure VS Code processes the 0 value before clearing
          setTimeout(() => {
            try {
              if (this._view) {
              ...
              console.warn(
                "[FlowCommand] Failed to clear badge (undefined set):",
                err,
              );
            }
          }, 10);
        ...
        console.warn("[FlowCommand] Badge API failure:", err);
      }
      ...
      try {
        if (total > 0) {
          ...
          // Force clear by setting to 0 explicitly first, then undefined
          try {
            this._viewForBadge.badge = { value: 0, tooltip: "" };
          } catch {
            // Ignore intermediate set error
          }
          setTimeout(() => {
            try {
              if (this._viewForBadge) {
                this._viewForBadge.badge = undefined;
              }
            } catch (err) {
              console.warn(
                "[FlowCommand] Failed to clear fallback badge:",
                err,
              );
            }
          }, 10);
  ...
  public async waitForUserResponse(
    ...
    if (!this._view) {
      // Open the FlowCommand sidebar view (conditionally focus based on auto-focus setting)
      if (this._autoFocusPanelEnabled) {
        await vscode.commands.executeCommand("flowCommandView.focus");
      } else {
        // Reveal the container without forcing focus on the view
        await vscode.commands.executeCommand(
          "workbench.view.extension.flowCommandContainer",
        );
      }
    ...
    if (this._webviewReady && this._view) {
      ...
      this._pendingToolCallMessage = {
        ...
        context: context,
        explicitChoices:
          choices.length > 0 && explicitChoices ? choices : undefined,
      };
    }
  ...
  private _handleSubmit(value: string, attachments: AttachmentInfo[]): void {
    if (this._pendingRequests.size > 0 && this._currentToolCallId) {
      ...
      if (resolve) {
        ...
        console.warn(
          `[FlowCommand] _handleSubmit: resolve not found for ${this._currentToolCallId}, clearing stale state`,
        );
        this._currentToolCallId = null;
        this._updateBadge();
   ...
   */
  private async _handleSearchContext(
    query: string,
    isRemote: boolean = false,
  ): Promise<void> {
    try {
...
<body>
  ...
  private _parseChoices(text: string): ParsedChoice[] {
    ...
    // Use negative lookahead instead of character class toavoid linting issues with combined characters
    const inlineEmojiPattern =
      /([0-9])\uFE0F?\u20E3\s+((?:(?!\s*[0-9]\uFE0F?\u20E3|[.!]\s+(?:Wait|wait|Please|please|Then|then|Select|select)|[.?!]\s*$).)+?)(?=\s*[0-9]\uFE0F?\u20E3|[.!]\s+(?:Wait|wait|Please|please|Then|then|Select|select)|[.?!]\s*$)/g;
